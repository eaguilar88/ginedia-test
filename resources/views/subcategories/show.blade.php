    ...
    <section class="container wrapper" id="subcategoria">
        ...
        <article class="display">
            ...
            <figure class="portrait">
                <img
                    src="{{ asset("storage/images/" . $subcategory->category->slug . "/$subcategory->slug/$subcategory->slug.jpg") }}">
                <figcaption>{{ $subcategory->name }}</figcaption>
            </figure>
            <ul class="calabaza v-flex">
                @foreach($products as $index => $product)
                    <li class="box">
                        <a href="{{ asset($product->zoom) }}" data-featherlight="{{url($product->zoomPath)}}"></a>
